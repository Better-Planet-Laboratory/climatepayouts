...
units_sf <- st_read(units_path, quiet = TRUE)  %>% unique()
admin_lookup <- units_sf %>% select(GID_0, NAME_0, NAME_1) %>% unique()
units_sf <- units_sf %>%
  select(-NAME_0, -year, -value, -farm_sz) %>% unique()
...
all_fs_pred <- read_csv("inputdata/farmsize_predictions.csv")
all_fs_pred <- all_fs_pred %>% filter(year==2020) %>% select(-year)
names(all_fs_pred)[names(all_fs_pred) == 'value'] <- 'frm2020'
...
#load loss and damage predictions
all_fs_pred <- read_csv("inputdata/farmsize_predictions.csv")
all_fs_pred <- all_fs_pred %>% filter(year==2050) %>% select(-year)
names(all_fs_pred)[names(all_fs_pred) == 'value'] <- 'frm2050'
...
fs_shp <- fs_shp %>% select(-farm_sz, -year, -value) %>% unique()

all_joint_shp <- left_join(all_joint, fs_shp, by=c("GID_0", "nm_slc0", "NAME_0", "NAME_1"))
all_joint_shp$model <- "baseline"
st_write(all_joint_shp, "intermediate/fig3_map_spei12_baseline_high.shp", delete_dsn = T)
